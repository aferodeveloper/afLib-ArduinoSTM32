...
// Tie this pin to the RESET line on Modulo
#define RESET                     PA1    // This pin is used to reboot the Modulo when needed
...
void setup() {
  stm32_logger_start(&Serial1, DEBUG_BAUD_RATE);
  ...
  */
  ...
  af_transport_t *platformUART = stm32_transport_create_uart(&Serial2, UART_BAUD_RATE);
  ...
  af_transport_t* platformSPI = stm32_transport_create_spi(CS_PIN);
  af_lib = af_lib_create_with_unified_callback(attrEventCallback, platformSPI);
  stm32_spi_setup_interrupts(af_lib, digitalPinToInterrupt(INT_PIN));
#else

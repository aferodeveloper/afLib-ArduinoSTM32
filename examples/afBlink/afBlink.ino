...
/* **************** STM32 Notes ***************
 *  Hardware Serial Ports for STM32F103:
 *
 *  "Serial1" (USART1) pins PA9, PA10 (TX, RX)
 *  "Serial2" (USART2) pins PA2, PA3 (TX, RX)
 *
 *  Serial/USART1 is usually the serial console/debug output
 *  Prefer using Serial2 for ASR.
 *
 *  Other STM32 Serial ports should work, but have not been tested.
 */
...
#include "profile/afBlink_Plumo-2D_UART/device-description.h"         // For Plumo-2D, UART
...
#define BLINK_INTERVAL            2000  // 2000 milliseconds, or 2 seconds
...
void setModuloLED(bool state) {
  if (moduloLEDState != state) {
    int result = af_lib_set_attribute_16(af_lib, AF_MODULO_LED, state, AF_LIB_SET_REASON_LOCAL_CHANGE);
    if (result != AF_SUCCESS) {
      // A more robust app would retry or act on the specific error returned
      af_logger_print_buffer("Set LED returned error: ");
...
void attrEventCallback(const af_lib_event_type_t eventType,
  ...
  switch (eventType) {
    ...
    case AF_LIB_EVENT_MCU_GET_REQUEST:
      switch (attributeId) {
        case AF_BLINK:
          af_lib_set_attribute_bool(af_lib, AF_BLINK, blinking, AF_LIB_SET_REASON_GET_RESPONSE);
          break;
        default:
          break;
      }
...
void setup() {
  ...
  // The use of af_logger here isn't necessary - common Arduino "Serial.println()"
  // debugging is probably the simplest, but af_logger is provided to allow a platform-
  // agnostic logging utility. It's included here (as opposed to straight "Serial" logging
  // of the original afBlink) just to test that it works.
  stm32_logger_start(&Serial1, 115200);
  af_logger_println_buffer("Starting afBlink");
  ...
  af_logger_println_buffer("Resetting ASR");
...
void printAttrBool(const char* attrLabel, const uint16_t attributeId, const uint16_t valueLen,
                   const uint8_t* value) {
...
void printAttr16(const char* attrLabel, const uint16_t attributeId, const uint16_t valueLen,
                 const uint8_t* value) {
  getPrintAttrHeader(attrLabel, attributeId, valueLen);
  if (valueLen > 0) {
    char intStr[6];
...
void printAttrStr(const char* attrLabel, const uint16_t attributeId, const uint16_t valueLen,
                  const uint8_t* value) {
  getPrintAttrHeader(attrLabel, attributeId, valueLen);
